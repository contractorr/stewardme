...
- **cli/** — Click CLI (`coach` command). Commands: ask, review, goals, journal, recommend, research, trends, mood, reflect, export, daemon, init. Pydantic config validation. Structlog logging
...
`src/coach_mcp/` exposes core modules as 22 MCP tools (journal, goals, intel, recommendations, research, reflection, mood). Claude Code does the reasoning; MCP server provides data + context retrieval only — no LLM calls in the MCP layer.
