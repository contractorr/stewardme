...
class WhyNowReasoner:
    ...
    def explain_suggestion(self, suggestion: dict, user_context: dict) -> list[dict]:
        ...
        if kind == "stale_goal":
            chips.append({"code": "stale_goal", "label": "Goal needs attention", "severity": "info", "detail": {}})

        if any(title and label.lower() in title for label in user_context.get("watchlist_labels") or []):
            matches = [label for label in user_context.get("watchlist_labels") or [] if label.lower() in title]
            chips.append(
                {
        ...
        if any(thread.lower() in title for thread in user_context.get("thread_labels") or []):
            chips.append({"code": "thread_reactivated", "label": "Recurring thread active", "severity": "info", "detail": {}})
    ...
    def explain_recommendation(self, recommendation: dict, user_context: dict) -> list[dict]:
        ...
        if harvested.get("state") == "positive":
            chips.append({"code": "executed_similar", "label": "Similar action paid off", "severity": "success", "detail": harvested})

        title = f"{recommendation.get('title','')} {recommendation.get('description','')}".lower()
        recent_hits = [item for item in user_context.get("recent_intel") or [] if any(term in title for term in str(item.get("title") or "").lower().split()[:6])]
        if recent_hits:
            chips.append(
                {
                    "code": "recent_intel",
                    ...
                    "severity": "info",
                    "detail": {"source_urls": [item.get("url") for item in recent_hits if item.get("url")]},
                }
            )
